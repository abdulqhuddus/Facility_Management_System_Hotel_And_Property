...
if(ISSET($_POST['submit'])){
    ...
    $sql_file = "SELECT * FROM `maintenance` WHERE invoice_id='".$_POST['contract_number']."'";
    $query_file = mysqli_query($conn, $sql_file);
    $row_file = mysqli_fetch_assoc($query_file);
    $last_id = $row_file['id'];
    $new_name = "MNTC_LIFT_BAL_DOC" . ($last_id + 1);
    $invoice_id = "MNTC_LIFT_BAL_INV" . ($last_id + 1);
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx'])) {
        ...
        $_SESSION['contract_number'] = $_POST['contract_number'];
